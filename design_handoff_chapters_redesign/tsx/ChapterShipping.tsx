// components/chapters/ChapterShipping.tsx (Chapter IV — side projects.)
"use client";
...
export function ChapterShipping() {
  ...
  return (
    <section
      ref={ref}
      id="shipping"
      className="relative isolate flex min-h-screen items-stretch bg-slate-950 text-white"
    >
      <Numeral dark position={{ bottom: -150, left: -50 }}>IV</Numeral>

      <div className="relative z-10 grid h-full w-full grid-cols-[1fr_1.4fr] gap-10 px-16 py-14">
        {/* left column */}
        <motion.div
        ...
        >
          ...
          <h2 className="text-[84px] font-semibold leading-[0.98] tracking-[-0.03em]">
            side<br />
            <em className="font-serif font-normal italic text-sky-300">projects.</em>
          </h2>
          <p className="text-white/80 text-[15.5px] leading-relaxed">
            Where I learn the stack outside of work — solo, evenings and weekends,
            no excuses. Cursor + Claude Code, ship to TestFlight, put it in front of
            real users.
          </p>
          <div className="mt-2 flex flex-wrap gap-2">
            <Pill dark dot><b className="font-semibold text-white tabular-nums">2</b>&nbsp;active builds</Pill>
            <Pill dark ghost>solo</Pill>
          </div>
          <motion.div
            style={reduced ? undefined : { opacity: sceneOpacity, clipPath: sceneClip }}
            className="relative mt-3 flex-1 min-h-0 overflow-hidden rounded-[18px] ring-1 ring-white/10"
          >
            <Image
