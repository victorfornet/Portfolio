// components/chapters/ChapterFoundation.tsx (Chapter I — hec paris.)
"use client";
...
export function ChapterFoundation() {
  ...
  return (
    ...
    >
      <Numeral position={{ bottom: -160, left: -40 }} className="text-[rgba(19,102,176,0.07)]">I</Numeral>
      ...
      <div className="grid h-full w-full grid-cols-1 md:grid-cols-[1fr_560px]">
        ...
        >
          ...
          <h2 className="text-[96px] font-semibold leading-[0.98] tracking-[-0.03em] text-slate-900">
            hec <em className="font-serif font-normal not-italic italic text-sky-700">paris.</em>
          </h2>
          <p className="max-w-[50ch] text-slate-700 text-[15.5px] leading-relaxed">
            Master in Management at{" "}
            <a href="https://www.hec.edu/en" target="_blank" rel="noreferrer" className={LINK}>
              HEC Paris
            </a>
            , ranked #1 worldwide by the Financial Times in 2025. Before that, three years
            in a French preparatory class (CPGE ECS), top 100 out of 9,000 candidates at
            the national entrance exam. One semester at Thammasat in Bangkok. Currently
            on a gap year to ship.
          </p>
          <motion.div
          ...
          >
            {stats.map((s) => (
              <Pill key={s.value}>
                <b className="font-semibold text-slate-900 tabular-nums">{s.value}</b>&nbsp;
                {s.label}
              </Pill>
            ))}
